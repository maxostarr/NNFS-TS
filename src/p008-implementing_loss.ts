...
abstract class Loss {
  calculate(output: tf.Tensor2D, y: tf.Tensor<tf.Rank.R1 | tf.Rank.R2>) {
    const sample_loss: tf.Tensor = this.forward(output, y);
...
class Loss_CategoricalCrossEntropy extends Loss {
  forward(
    ...
    if (y_true.shape.length === 1) {
      correct_confidences = tf.stack(
        (y_true as unknown as tf.Tensor1D)
          .arraySync()
          .map((v, i) => y_pred_clipped.slice([i, v], [1, 1]).squeeze()),
      );
    }
    if (y_true.shape.length === 2) {
      correct_confidences = tf.sum(y_pred_clipped.mul(y_true), 1);
    }
    if (!correct_confidences) throw new Error("y_true dims invalid");
    const loss = tf.log(correct_confidences).neg();
    return loss;
  }
}
...
// const loss = loss_function.calculate(output_test, y_test_one_hot);
...
const X_tensor = tf.tensor2d(X.toArray());
const y_tensor = tf.tensor1d(y.toArray().flat(), "int32");
y_tensor.print();
X_tensor.print();
...
const loss_function = new Loss_CategoricalCrossEntropy();
const loss = loss_function.calculate(activation2.output, y_tensor);
